...
function NewMovie({onAddMovie}) {
    ...
    useEffect(() => {
        setNewMovie({id, title, year, category, price, imageSrc});
    }, [id, title, year, category, price, imageSrc]);
    ...
    <form onSubmit={handleSubmit}>
        ...
        <input type="file" id="add-movie-poster" onChange={(e)=>setImageSrc(e.target.files[0].name)}></input>
        <input type="submit" id="add-movie-btn" value="Add a Movie" />
    </form>
    );
}

function ReactRouterMovieLocal() {
...
export default ReactRouterMovieLocal;